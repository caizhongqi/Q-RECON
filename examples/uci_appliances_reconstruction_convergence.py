...
def _scalar(values: list[float], label: str) -> dict[str, object] | None:
    ...
    return summarize_scalar(
        ...
        label=f"uci-reconstruction-convergence:{label}",
    ).to_dict()
...
def _step_summary(
    ...
    return {
        ...
        "ordered_input_mse": _scalar(
            ordered_mse, f"{architecture}:{steps}:ordered-mse"
        ),
        "channel_aligned_input_mse": _scalar(
            aligned_mse, f"{architecture}:{steps}:aligned-mse"
        ),
        "ordered_input_relative_l2": _scalar(
            ordered_relative, f"{architecture}:{steps}:ordered-relative"
        ),
        "channel_aligned_input_relative_l2": _scalar(
            aligned_relative, f"{architecture}:{steps}:aligned-relative"
        ),
        "ordered_input_correlation": _scalar(
            ordered_correlation, f"{architecture}:{steps}:ordered-correlation"
        ),
        "channel_aligned_input_correlation": _scalar(
            aligned_correlation, f"{architecture}:{steps}:aligned-correlation"
        ),
        "best_objective": _scalar(
            [float(item["best_objective"]) for item in selected],
            f"{architecture}:{steps}:best-objective",
        ),
        "best_step": _scalar(
            [float(item["best_step"]) for item in selected],
            f"{architecture}:{steps}:best-step",
        ),
    }
...
def _paired_delta(
    ...
    left_steps: int,
    right_steps: int,
    selected_by_steps: dict[int, list[dict[str, object]]],
) -> dict[str, object]:
    left = {int(item["batch_index"]): item for item in selected_by_steps[left_steps]}
    right = {int(item["batch_index"]): item for item in selected_by_steps[right_steps]}
    ...
    label = f"{architecture}:{right_steps}-minus-{left_steps}"
    return {
        "from_steps": left_steps,
        "to_steps": right_steps,
...
def main() -> None:
    ...
    if task != "forecasting" or mode != "timeseries":
        raise RuntimeError("convergence study requires forecasting data")
    ...
    selected_by_steps: dict[int, list[dict[str, object]]] = {
        steps: [] for steps in STEP_COUNTS
    }
    selected_indices: dict[str, list[int]] = {
        str(steps): [] for steps in STEP_COUNTS
    }
    ...
    for steps in STEP_COUNTS:
        for batch_index in ATTACK_INDICES:
            ...
            if chosen is None:
                ...
                selected_indices[str(steps)].append(chosen)
                selected_by_steps[steps].append(attempts[chosen])

    failed_attempts = sum(item["status"] != "success" for item in attempts)
    summaries = {
        str(steps): _step_summary(
            architecture, steps, selected_by_steps[steps]
        )
        for steps in STEP_COUNTS
    }
    paired = {
        "100_minus_25": _paired_delta(
            architecture, 25, 100, selected_by_steps
        ),
        "400_minus_100": _paired_delta(
            architecture, 100, 400, selected_by_steps
        ),
        "400_minus_25": _paired_delta(
            architecture, 25, 400, selected_by_steps
        ),
    }

    expected_attempts = len(ATTACK_INDICES) * len(RESTART_SEEDS) * len(STEP_COUNTS)
    all_step_groups_complete = all(
        len(selected_by_steps[steps]) == len(ATTACK_INDICES)
        for steps in STEP_COUNTS
    )
    quality_gate_passed = (
        ...
        and STEP_COUNTS == (25, 100, 400)
        and len(attempts) == expected_attempts
        and failed_attempts == 0
        and selection_complete
        and all_step_groups_complete
    )

    payload = {
        "schema_version": "qrecon.uci-reconstruction-convergence.v1",
        ...
        "attack_contract": {
            ...
            "selection_rule": "minimum released-gradient objective within step count",
            "known_target": False,
            "match_mode": "hybrid",
            "layer_weighting": "parameter",
        ...
        "quality_gate": {
            ...
            "three_restarts_per_step_count": len(RESTART_SEEDS) == 3,
            "predeclared_25_100_400_steps": STEP_COUNTS == (25, 100, 400),
            "all_ninety_attempts_completed": len(attempts) == 90
            and failed_attempts == 0,
            "every_record_selected_at_every_step_count": selection_complete
            and all_step_groups_complete,
            "passed": quality_gate_passed,
        },
        "environment": benchmark_environment_manifest(),
        "interpretation": (
            "The convergence sweep keeps the dataset, model, private records, restart "
            "seeds, attack objective and selection rule fixed while increasing only the "
            "optimization budget. Ordered and permutation-aligned metrics are both "
            "reported; the alignment is evaluation-only and uses the private reference "
            "after restart selection."
        ),
        "claim_boundary": (
            "Failure to improve with more iterations is evidence about this optimizer and "
            "configuration, not an information-theoretic impossibility theorem. Conversely, "
            "a lower aligned error does not identify semantic channel order or establish "
            "quantum advantage. GitHub-hosted timing is not hardware-speedup evidence."
        ),
    }
    print(json.dumps(payload, indent=2, sort_keys=True))
    if not quality_gate_passed:
        raise SystemExit("UCI reconstruction convergence quality gate failed")
