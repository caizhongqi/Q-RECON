...
from qrecon.benchmarks.paired_learned_ts_inverse import (
...
def main() -> None:
    ...
    payload["publication_claim"] = {
        ...
        "comparison": "random_l1 versus learned_quantile_l1",
        "paired_batches": len(manifest.attack_indices),
        "restart_seeds_per_method": len(manifest.attack_seeds),
        "claim_boundary": (
            "Both methods use the same victim, gradients, attacked records, seeds, "
            "optimization objective and step budget. The learned method alone uses a "
            "gradient-to-quantile network trained on a disjoint auxiliary split. The "
            "study is known-target and does not establish quantum advantage."
        ),
    }
    print(json.dumps(payload, indent=2, sort_keys=True))
    if not report.quality_gate.passed:
        raise SystemExit("paired learned TS-Inverse quality gate failed")
